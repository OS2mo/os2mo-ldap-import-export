...
The integration is configured using environment variables (see `mo_ldap_import_export/config.py` for details).
...
3. FastRAMQPI picks up the event and sends a HTTP request to the integration.
...
- NEVER edit `autogenerated_graphql_client/` manually.
