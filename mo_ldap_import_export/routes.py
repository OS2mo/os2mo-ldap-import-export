...
def _outliving_uuids(objects: list, termination_date: datetime) -> set[UUID]:
    """UUIDs of MO objects whose latest validity ends after `termination_date`."""
    return {
        last(obj.validities).uuid
        for obj in objects
        if (to := last(obj.validities).validity.to) is None or to > termination_date
    }
...
async def get_dangling_ituser_relations(
    ...
    async for ituser in paged_query(read):
        ...
        if termination_date is None:
            continue
        # `from_date=None, to_date=None` widens the window to all of time so
        # the terminated IT-user (whose `gyldighed` is `Inaktiv` now) resolves.
        link = ITUserFilter(uuids=[ituser.uuid], from_date=None, to_date=None)
        engagements = await graphql_client.read_engagements(
            EngagementFilter(ituser=link, from_date=None, to_date=None)
        )
        addresses = await graphql_client.read_filtered_addresses(
            AddressFilter(ituser=link, from_date=None, to_date=None)
        )
        dangling_engagements = _outliving_uuids(engagements.objects, termination_date)
...
def construct_router(settings: Settings) -> APIRouter:
    ...
    @router.post(
        "/fixup/terminate_dangling_it_user_relations",
        status_code=200,
        tags=["LDAP"],
    )
    async def terminate_dangling_it_user_relations(
        graphql_client: depends.GraphQLClient,
        it_system: UUID,
        dry_run: bool = True,
    ) -> dict[str, dict[str, Any]]:
        ...
        if dry_run:
            return dangling
        for info in dangling.values():
            for uuid in info["engagements"]:
                await graphql_client.engagement_terminate(
                    EngagementTerminateInput(uuid=uuid, to=info["termination_date"])
                )
            for uuid in info["addresses"]:
                await graphql_client.address_terminate(
                    AddressTerminateInput(uuid=uuid, to=info["termination_date"])
                )
        return dangling
