...
from .ldap_event_generator import LastRun
...
@sync_router.post("/ldap2mo")
async def sync_ldap2mo(
    ldap_event_generator: depends.LDAPEventGenerator,
) -> None:
    """Trigger a full LDAP -> OS2mo resync.

    Clears the LDAP poller's bookkeeping so the next poll treats every entry
    in the configured search bases as new and emits events for them.
    """
    async with ldap_event_generator.sessionmaker() as session, session.begin():
        await session.execute(delete(LastRun))


@sync_router.post("/mo2ldap")
async def sync_mo2ldap(
    graphql_client: depends.GraphQLClient,
) -> None:
    """Trigger a full OS2mo -> LDAP resync.

    For every listener this integration owns in the "mo" namespace, fires the
    matching `<routing_key>_refresh_all` mutator on OS2mo. The set of
    routing keys is derived from CONVERSION_MAPPING.mo_to_ldap together with
    the static listeners registered at startup.
    """
    ...
    for listener in listeners.objects:
        method = getattr(graphql_client, f"{listener.routing_key}_refresh_all", None)
        if method is None:
            logger.warning(
                "No refresh-all mutator for routing_key",
                routing_key=listener.routing_key,
                listener=listener.uuid,
            )
            continue
        await method(listener=listener.uuid, owner=me.actor.uuid)
