...
def construct_router(settings: Settings) -> APIRouter:
    ...
    @router.get("/Inspect/mo_to_ldap/all", status_code=200, tags=["LDAP"])
    async def mo_to_ldap_templating_all(
        graphql_client: depends.GraphQLClient,
        sync_tool: depends.SyncTool,
        ...
        from .main import mo_to_ldap_handler
        ...
        for mapping in settings.conversion_mapping.mo_to_ldap:
            if mapping.routing_key == "person":
                ...
                employee_handlers.append((mapping.identifier, handler))

        async def process_uuid(uuid: UUID) -> list[dict[str, Any]]:
            event = Event(
                uuid=uuid,
            ...
            results = []
            for identifier, handler in employee_handlers:
                try:
                    await handler(
                        exit_stack=exit_stack,
                        converter=sync_tool.converter,
                        dataloader=sync_tool.dataloader,
                        event=event,
                        dry_run=True,
                    )
                    results.append(
                    ...
                    logger.exception("Exception during Inspect/mo_to_ldap/all")
                    results.append(
                        {
                            "__mo_uuid": uuid,
                            "identifier": identifier,
        ...
        with open("/tmp/mo_to_ldap.jsonl", "w") as fout:
            import json

            for uuid in uuids:
                contexts = await process_uuid(uuid)
    ...
    @router.get("/Inspect/mo_to_ldap/ituser/all", status_code=200, tags=["LDAP"])
    async def mo_to_ldap_templating_ituser_all(
        graphql_client: depends.GraphQLClient,
        sync_tool: depends.SyncTool,
        settings: depends.Settings,
        exit_stack: depends.ExitStack,
        ...
        cursor = None
        uuid_to_employee_uuid: dict[UUID, UUID] = {}
        while True:
            result = await graphql_client.read_all_itusers(
                filter=ITUserFilter(), cursor=cursor
            )
            for obj in result.objects:
                for val in obj.validities:
                    if val.uuid:
                        uuid_to_employee_uuid[val.uuid] = val.employee_uuid

            if not result.page_info.next_cursor:
                break
            cursor = result.page_info.next_cursor

        uuids = list(uuid_to_employee_uuid.keys())
        if start_at:
            uuids = [uuid for uuid in uuids if uuid > start_at]
        ...
        ituser_handlers = []
        for mapping in settings.conversion_mapping.mo_to_ldap:
            if mapping.routing_key == "ituser":
                handler = mo_to_ldap_handler(
                    mapping.identifier, mapping.template, mapping.object_class
                )
                ituser_handlers.append((mapping.identifier, handler))
        ...
        async def process_uuid(uuid: UUID) -> list[dict[str, Any]]:
            ...
            results = []
            for identifier, handler in ituser_handlers:
                try:
                    await handler(
                        exit_stack=exit_stack,
                        converter=sync_tool.converter,
                    ...
                    results.append(
                        {
                            "__mo_uuid": uuid,
                            "__mo_employee_uuid": uuid_to_employee_uuid.get(uuid),
                            "identifier": identifier,
                            "message": "No changes",
                        }
                    )
                    ...
                    logger = structlog.get_logger()
                    logger.exception("Exception during Inspect/mo_to_ldap/ituser/all")
                    results.append(
                        {
                            "__mo_uuid": uuid,
                            "__mo_employee_uuid": uuid_to_employee_uuid.get(uuid),
        ...
        with open("/tmp/mo_to_ldap_ituser.jsonl", "w") as fout:
            import json
    ...
    async def _process_mo_to_ldap_type(
        type_: str,
        graphql_client: depends.GraphQLClient,
        sync_tool: depends.SyncTool,
        settings: depends.Settings,
        ...
        uuid_context: dict[UUID, dict[str, Any]] = {}
        if type_ == "person":
            result = await graphql_client.read_person_uuid()
            for obj in result.objects:
                uuid_context[obj.uuid] = {}
        elif type_ == "ituser":
            from mo_ldap_import_export.autogenerated_graphql_client import ITUserFilter
            ...
            while True:
                ...
                for obj in result.objects:
                    for val in obj.validities:
                        if val.uuid:
                            uuid_context[val.uuid] = {
                                "__mo_employee_uuid": val.employee_uuid
                            }
                if not result.page_info.next_cursor:
                    break
                cursor = result.page_info.next_cursor
        else:
            raise HTTPException(status_code=404, detail=f"Unsupported type: {type_}")

        uuids = list(uuid_context.keys())
        if start_at:
            uuids = [u for u in uuids if u > start_at]

        handlers = []
        for mapping in settings.conversion_mapping.mo_to_ldap:
            if mapping.routing_key == type_:
                handler = mo_to_ldap_handler(
                    mapping.identifier, mapping.template, mapping.object_class
                )
                handlers.append((mapping.identifier, handler))
        ...
        async def process_uuid(uuid: UUID) -> list[dict[str, Any]]:
            ...
            for identifier, handler in handlers:
                ...
                try:
                    ...
                    results.append({**base_result, "message": "No changes"})
                except DryRunException as exc:
                    assert isinstance(exc.detail, dict)
                    results.append({**base_result, **exc.detail})
                except Exception:  # pragma: no cover
                    import structlog

                    logger = structlog.get_logger()
                    logger.exception(f"Exception during Inspect/mo_to_ldap/{type_}/all")
                    results.append(
                        {**base_result, "message": "Exception during processing"}
                    )
            return results

        filename = (
            f"/tmp/mo_to_ldap_{type_}.jsonl"
            if type_ != "person"
            else "/tmp/mo_to_ldap.jsonl"
        )
    ...
    @router.get("/Inspect/mo_to_ldap/all", status_code=200, tags=["LDAP"])
    async def mo_to_ldap_templating_all_types(
        graphql_client: depends.GraphQLClient,
        sync_tool: depends.SyncTool,
        settings: depends.Settings,
        exit_stack: depends.ExitStack,
        ...
        types = {
            mapping.routing_key for mapping in settings.conversion_mapping.mo_to_ldap
        }
        for type_ in types:
            try:
                await _process_mo_to_ldap_type(
                    type_=type_,
    ...
    @router.get("/Inspect/mo_to_ldap/{type_}/all", status_code=200, tags=["LDAP"])
    async def mo_to_ldap_templating_by_type(
        type_: str,
        graphql_client: depends.GraphQLClient,
        sync_tool: depends.SyncTool,
        settings: depends.Settings,
