...
from .autogenerated_graphql_client.read_current_addresses_filtered_with_pagination import ReadCurrentAddressesFilteredWithPaginationAddressesObjects
...
class AddressValidation(BaseModel):
    address_uuid: UUID
    has_it_user: bool
    it_user_uuid: UUID | None
    has_engagement: bool
    ldap_uuid: str | None
    set_for_deletion: bool


    @classmethod
    def from_obj_in_read_current_addresses_filtered_with_pagination(cls: type['AddressValidation'], address: ReadCurrentAddressesFilteredWithPaginationAddressesObjects) -> 'AddressValidation':
        if address.current is None:
            raise ValueError("address.current is None")
        current = address.current
        address_uuid = current.uuid
        if current.ituser_response is None or current.ituser_response.current is None:
            has_it_user = False
            it_user_uuid = None
            ldap_uuid = None
            ...
            has_it_user = True
            it_user_uuid = current.ituser_response.current.uuid
            ldap_uuid = current.ituser_response.current.external_id
            has_engagement = False

            for eng in current.ituser_response.current.engagements:
        ...
        return AddressValidation(
            ...
            has_it_user=has_it_user,
            it_user_uuid=it_user_uuid,
            has_engagement=has_engagement,
            ldap_uuid=ldap_uuid,
            set_for_deletion= not (has_it_user and has_engagement),
        )
