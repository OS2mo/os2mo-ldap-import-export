...
def mo_to_ldap_handler(
    ...
    async def inner(
        ...
        assert "dn" in ldap_desired_state, "Missing 'dn' in ldap_desired_state"
        dn = one(ldap_desired_state["dn"])
        assert isinstance(dn, str), "dn must be a string"

        assert "create" in ldap_desired_state, "Missing 'create' in ldap_desired_state"
        create = one(ldap_desired_state["create"])
        assert isinstance(create, bool), "create must be a boolean"

        assert (
            "attributes" in ldap_desired_state
        ), "Missing 'attributes' in ldap_desired_state"
        attributes = one(ldap_desired_state["attributes"])
        assert isinstance(attributes, dict), "attributes must be a dictionary"

        ldapapi = converter.dataloader.ldapapi
        await ldapapi.ensure_ldap_object(dn, attributes, object_class, create)
