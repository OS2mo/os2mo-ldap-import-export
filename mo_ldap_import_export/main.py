...
from .import_export import SyncTool, with_exitstack
from .ldap import check_ou_in_list_of_ous, get_ldap_object
...
async def ensure_ldap_object(
    ...
    if not current_dn:
        await ldapapi.add_ldap_object(desired_dn, attributes, object_class)
        return
    ...
    # TODO: This should be a function on LDAPAPI
    ...
    await ldapapi.modify_ldap_object(current_dn, ldap_changes)


@with_exitstack
async def mo_to_ldap_handler(
    identifier: str,
    template_string: str,
    ...
    exit_stack.enter_context(bound_contextvars(identifier=identifier, uuid=str(uuid)))
    ...
    ldap_desired_state = {key: ensure_list(value) for key, value in parsed.items()}

    if not ldap_desired_state:
        logger.info("Not writing to LDAP as changeset is empty")
        return

    if "object_class" not in ldap_desired_state:
        logger.error("Missing 'object_class' in ldap_desired_state")
        raise RequeueMessage("Integration must be reconfigured")
    object_class = one(ldap_desired_state["object_class"])

    if "desired_dn" not in ldap_desired_state:
        logger.error("Missing 'desired_dn' in ldap_desired_state")
        raise RequeueMessage("Integration must be reconfigured")
    desired_dn = one(ldap_desired_state["desired_dn"])

    current_dn = only(ldap_desired_state.get("current_dn", []))
    attributes = ldap_desired_state.get("attributes")
    await ensure_ldap_object(ldapapi, object_class, current_dn, desired_dn, attributes)
    


def construct_mapping_router(settings: Settings) -> MORouter:
    router = MORouter()
    for mapping in settings.conversion_mapping.mo_to_ldap:
        handler = partial(mo_to_ldap_handler, mapping.identifier, mapping.template)
        handler.__name__ = mapping.identifier  # type: ignore
        router.register(mapping.routing_key)(handler)
    return router
...
def create_fastramqpi(**kwargs: Any) -> FastRAMQPI:
    ...
    if settings.listen_to_changes_in_mo:
        ...
        mapping_router = construct_mapping_router(settings)
        amqpsystem.router.registry.update(mapping_router.registry)
