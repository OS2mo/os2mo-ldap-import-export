...
from ..autogenerated_graphql_client.input_types import AddressFilter, RoleBindingFilter
...
async def get_class_user_key(dataloader: DataLoader, uuid: UUID) -> str | None:
    result = await dataloader.moapi.load_mo_class(uuid)
    return None if result is None else result.user_key


async def get_itsystem_user_key(dataloader: DataLoader, uuid: UUID) -> str | None:
    result = await dataloader.moapi.load_mo_it_system(uuid)
    return None if result is None else result.user_key 


async def rolebinding_uuid_to_role_uuid(graphql_client: GraphQLClient, uuid: UUID) -> UUID | None:
    result = await graphql_client.read_rolebindings(filter=RoleBindingFilter(
        uuids=[uuid]
    ))
    rolebinding = only(result.objects)
    if rolebinding is None:
        return None
    if rolebinding.current is None:
        return None
    role_validities = rolebinding.current.role
    role_uuid = one({role.uuid for role in role_validities})
    return role_uuid


async def rolebinding_uuid_to_ituser_uuid(graphql_client: GraphQLClient, uuid: UUID) -> UUID | None:
    result = await graphql_client.read_rolebindings(filter=RoleBindingFilter(
        uuids=[uuid]
    ))
    rolebinding = only(result.objects)
    ...
    if rolebinding.current is None:
        return None
    ituser_validities = rolebinding.current.ituser
    ituser_uuid = one({ituser.uuid for ituser in ituser_validities})
    return ituser_uuid


async def ituser_uuid_to_itsystem_uuid(dataloader: DataLoader, uuid: UUID) -> UUID | None:
    result = await dataloader.moapi.load_mo_it_user(uuid)
    return None if result is None else result.itsystem
...
def construct_globals_dict(
    ...
    return {
        ...
        "get_class_user_key": partial(get_class_user_key, dataloader),
        "get_itsystem_user_key": partial(get_itsystem_user_key, dataloader),
        "rolebinding_uuid_to_role_uuid": partial(rolebinding_uuid_to_role_uuid, graphql_client),
        "rolebinding_uuid_to_ituser_uuid": partial(rolebinding_uuid_to_ituser_uuid, graphql_client),
        "ituser_uuid_to_itsystem_uuid": partial(ituser_uuid_to_itsystem_uuid, dataloader),
    }
