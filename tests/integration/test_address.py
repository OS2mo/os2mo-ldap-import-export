...
async def test_to_mo(
    ...
    async def get_address() -> dict[str, Any] | None:
        ...
        address = only(addresses.objects)
        if address is None:
            return None
    ...
    async for attempt in retrying():
        with attempt:
            assert await get_address() is None
        ...
        "CONVERSION_MAPPING": json.dumps(
            {
                "ldap_to_mo": {
                    "EmailEmployee": {
                        ...
                        "_terminate_": "2860-11-12T12:12:42Z",
                        "uuid": "{{ get_address_uuid({'address_type': {'user_key': 'EmailEmployee'}, 'employee': {'uuids': [employee_uuid]}}) }}",
                        "user_key": "-",
                        "value": "{{ ldap.mail }}",
                        "address_type": "{{ get_employee_address_type_uuid('EmailEmployee') }}",
...
async def test_terminate_on_create(
    ...
    assert one(addresses.objects).dict() == {
        "validities": [
            {
                "uuid": ANY,
                "user_key": "-",
                ...
                "validity": {
                    "from_": mo_today(),
                    "to": datetime(2860, 11, 12, tzinfo=MO_TZ),
                },
            },
        ],
    }
...
@pytest.mark.usefixtures("test_client")
async def test_terminate_on_update(
    graphql_client: GraphQLClient,
    mo_person: UUID,
    email_employee: UUID,
    trigger_ldap_person: Callable[[], Awaitable[None]],
        ...
        "validities": [
            {
                ...
                "visibility": None,
                "validity": {
                    "from_": datetime(2000, 1, 1, tzinfo=MO_TZ),
                    "to": mo_today(),
                },
            },
            {
