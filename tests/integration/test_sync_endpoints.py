...
from mo_ldap_import_export.autogenerated_graphql_client import ListenerFilter
...
async def test_sync_mo2ldap_iterates_listeners(
    test_client: AsyncClient,
    graphql_client: GraphQLClient,
) -> None:
    """POST /sync/mo2ldap iterates the integration's MO listeners and returns 200."""
    ...
    assert listeners.objects, "Integration should have registered MO listeners"

    result = await test_client.post("/sync/mo2ldap")
    assert result.status_code == 200, result.text
