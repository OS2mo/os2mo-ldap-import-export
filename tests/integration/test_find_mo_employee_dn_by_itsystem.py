...
@pytest.fixture
async def mo_person_unique(graphql_client: GraphQLClient) -> UUID:
    r = await graphql_client.person_create(
        input=EmployeeCreateInput(
            given_name="Unique",
            surname="User",
            cpr_number="0101990101",
        )
    )
    return r.uuid
...
async def test_find_mo_employee_dn_by_itsystem_ituser_termination(
    ...
    with capture_logs() as cap_logs:
        dns = await sync_tool.dataloader.find_mo_employee_dn_by_itsystem(mo_person_unique)
        assert dns == set()
    assert cap_logs == []

    # Ensure that no link exists between the mo person and LDAP account
    ituser_map = await read_mo_mapping_uuids(mo_person_unique)
    assert ituser_map == {}

    # Create the link between the mo person and the LDAP account
    with capture_logs() as cap_logs:
        await sync_tool.ensure_ituser_link(mo_person_unique, dn)
    ...
    assert cap_logs == [
        ...
        {
            "dns": {dn},
            "employee_uuid": mo_person_unique,
