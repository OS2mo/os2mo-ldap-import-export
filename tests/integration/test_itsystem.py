...
async def test_to_mo(
    ...
    async def get_itsystem(user_key: str) -> dict[str, Any] | None:
        it_systems = await graphql_client._testing__itsystem_read(
            filter=ITSystemFilter(user_keys=[user_key])
        )
        it_system = only(it_systems.objects)
        if it_system is None:
            return None
    ...
    await trigger_ldap_sync(group_uuid)
    assert await get_itsystem(str(group_uuid)) is None
