        ...
        "CONVERSION_MAPPING": json.dumps(
            {
                "ldap_to_mo_any": {
                    ...
                    "groupOfNames": {
                        ...
                        "RoleBinding": {
                            "objectClass": "RoleBinding",
                            "_import_to_mo_": "true",
                            "_ldap_attributes_": ["cn", "entryUUID", "description", "member"],
                            "_terminate_": "{{ now()|mo_datestring if ldap.description == 'DISABLED' else '' }}",
                            "uuid": "{{ get_itsystem_uuid({'user_keys': [ldap.entryUUID + dn2uuid(ldap.member)]}) }}",
                            "user_key": "{{ ldap.entryUUID + dn2uuid(ldap.member) }}",
                            "ituser": ...,
                            "org_unit": ...,
                            "role": ...,
                        }
...
async def test_to_mo(
    ...
    async def get_role_binding(user_key: str) -> dict[str, Any]:
        rolebindings = await graphql_client._testing__rolebinding_read(
            filter=RoleBindingFilter(user_keys=[user_key])
        )
        rolebinding = one(rolebindings.objects)
        validities = one(rolebinding.validities)
        return validities.dict()
