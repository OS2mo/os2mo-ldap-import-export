...
async def test_load_it_user_deleted_return_terminated(
    ...
    assert result == ITUser(
        ...
        person=mo_person,
        validity=Validity(
            start=datetime(2001, 2, 3, tzinfo=MO_TZ),
            end=datetime(2002, 3, 4, tzinfo=MO_TZ),
        ),
    )
...
async def test_load_itusers_multiple_disjoint_matches(
    ...
    await graphql_client.ituser_create(
        input=ITUserCreateInput(
            ...
            itsystem=adtitle,
            person=mo_person,
            validity=RAValidityInput(from_="2003-04-05T06:07:08Z"),
        )
    )
