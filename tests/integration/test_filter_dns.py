...
async def test_filter_dns_no_filter(input_dns: set[DN]) -> None:
    ...
    assert input_dns == {obj.dn for obj in output_objects}
...
async def test_filter_dns(
    ...
    settings = Settings()
    ldap_objects = await ldap_api.get_objects_by_dns([ldap_person_dn])
    result = await filter_dns(
        settings, ldap_api.ldap_connection.connection, ldap_objects
    )
...
async def test_filter_dns_accepts_pre_moonlanding(
    ...
    settings = Settings()
    ldap_objects = await ldap_api.get_objects_by_dns({b18, b19, a19, a20})
    result = await filter_dns(
        settings, ldap_api.ldap_connection.connection, ldap_objects
    )
