...
async def test_non_existing_unique_ldap_uuids_itsystem_user_key_override(
    test_client: AsyncClient,
    graphql_client: GraphQLClient,
    mo_person: UUID,
) -> None:
    override = await graphql_client.itsystem_create(
        input=ITSystemCreateInput(
            user_key="OTHER_ADUUID",
            name="OTHER_ADUUID",
    ...
    response = await test_client.post(
        "/fixup/delete_non_existing_unique_ldap_uuids",
        params={"at": "2020-01-01T00:00:00", "itsystem_user_key": "OTHER_ADUUID"},
    )
    assert response.status_code == 200
