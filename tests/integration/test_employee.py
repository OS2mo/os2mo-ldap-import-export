...
async def test_generate_common_name(
    ...
    await graphql_client._testing__person_update(
        EmployeeUpdateInput(
            uuid=mo_person,
            surname="Klareng",
            validity=RAValidityInput(from_=mo_today()),
        )
    )
    await trigger_sync(mo_person)
    common_name = await fetch_common_name(mo_person_cpr_number)
...
async def test_generate_common_name_exhausted(
    context: Context,
    ...
    with pytest.raises(RuntimeError, match="Failed to create common name"):
        await username_generator._create_common_name(["Nick", "Johnson"])
