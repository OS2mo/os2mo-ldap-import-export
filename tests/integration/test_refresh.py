...
async def test_refresh_chain_graphql(
    ...
    borgerpc = await graphql_client.itsystem_create(
        input=ITSystemCreateInput(
            user_key="borgerpc",
            name="BorgerPC",
            validity=RAOpenValidityInput(from_="2001-02-03T04:05:06Z"),
        )
    )
    ...
    async with run_server(app):
        ...
        # This chain is two events deep, and every handler along it competes
        # for the single LDAP connection, so it can take a good while under
        # load. Increase the timeout.
