...
@pytest.mark.usefixtures("test_client")
async def test_poll_rejects_non_naming_context_search_base(
    context: Context,
) -> None:
    """DirSync requires the search base to be a naming context (root DN).

    Polling with a non-root base (here ``CN=Users,...``) must fail at runtime
    rather than silently returning partial or empty results.
    """
    settings = Settings(ldap_event_generator_type="dirsync")
    assert settings.ldap_search_base == "CN=Users,DC=magenta,DC=dk"
    ...
    try:
        with pytest.raises(Exception) as exc_info:
            await event_generator.poll(None)
        assert "not one of the naming context" in str(exc_info.value)
    finally:
        event_generator.dirsync_connection.unbind()
