...
async def test_write_manager_to_ldap(
    ...
    await graphql_client.engagement_create(
        input=EngagementCreateInput(
            ...
            job_function=jurist,
            primary=primary,
            validity=RAValidityInput(from_="2000-01-01T00:00:00Z"),
        )
    )
    ...
    manager_dn = one(ldap_object.manager)  # type: ignore
    assert isinstance(manager_dn, str)

    # Manually fetch manager object via its DN
    manager_object = await get_ldap_object(
        ldap_api.ldap_connection.connection, manager_dn
    )
