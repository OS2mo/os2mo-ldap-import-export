...
async def test_load_primary_engagement_ending_today(
    ...
    # Engagement starting tomorrow. MO snaps validity start dates to local
    # midnight, so `datetime.now(UTC) + timedelta(days=1)` may end up starting
    # today (Copenhagen time) when the test runs around midnight. Anchor the
    # start to tomorrow's Copenhagen midnight instead.
    tomorrow = mo_today() + timedelta(days=1)
    await graphql_client.engagement_update(
        input=EngagementUpdateInput(
