...
from mo_ldap_import_export.environments.main import get_manager_person_uuids
...
async def test_get_manager_person_uuid_current_engagement_no_manager(
    ...
    events = [m["event"] for m in cap_logs]
    assert events == ["No manager relation(s) found"]
...
async def test_get_manager_person_uuid_current_engagement_vacant_manager(
    ...
    events = [m["event"] for m in cap_logs]
    assert events == ["Vacant manager(s) found"]
...
@pytest.mark.usefixtures("test_client")
async def test_get_manager_person_uuids_vacant_and_active(
    graphql_client: GraphQLClient,
    create_engagement: CreateEngagement,
    create_manager: CreateManager,
) -> None:
    # Add a vacant manager
    engagement_uuid = await create_engagement(datetime(2000, 1, 1), None)
    await create_manager(None, datetime(2000, 1, 1), None)

    with capture_logs() as cap_logs:
        result = await get_manager_person_uuids(
            graphql_client, engagement_uuid=engagement_uuid
        )
    assert result == {None}
    ...
    assert result == {None, manager_person.uuid}
...
@pytest.mark.usefixtures("test_client")
async def test_get_manager_person_uuids_two_active(
    graphql_client: GraphQLClient,
    create_engagement: CreateEngagement,
    create_manager: CreateManager,
) -> None:
    # Add two managers
    engagement_uuid = await create_engagement(datetime(2000, 1, 1), None)
    manager_person1 = await graphql_client.person_create(
        input=EmployeeCreateInput(
            given_name="First",
            surname="Supervisor",
            cpr_number="0101701111",
        )
    )
    manager_person2 = await graphql_client.person_create(
        input=EmployeeCreateInput(
            given_name="Second",
            surname="Supervisor",
            cpr_number="0101702222",
        )
    )
    await create_manager(EmployeeUUID(manager_person1.uuid), datetime(2000, 1, 1), None)
    await create_manager(EmployeeUUID(manager_person2.uuid), datetime(2000, 1, 1), None)

    result = await get_manager_person_uuids(
        graphql_client,
        engagement_uuid=engagement_uuid,
    )
    assert result == {manager_person1.uuid, manager_person2.uuid}
