...
def pytest_configure(config: Config) -> None:
    config.addinivalue_line(
        "markers", "disable_fixture(name): Disable a fixture by name for a marked test"
    )
...
def pytest_collection_modifyitems(items: list[Item]) -> None:
    ...
    for item in items:
        ...
        # Remove requested fixture(s)
        for marker in item.iter_markers(name="disable_fixture"):
            assert marker.args, "disable_fixture: must be called with fixture name(s)"
            for fixture_to_disable in marker.args:
                assert (
                    fixture_to_disable in item.fixturenames
                ), "disable_fixture: cannot disable unused fixture"
                item.fixturenames.remove(fixture_to_disable)
